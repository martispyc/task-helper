...
UX layer (all local, zero network): next-action status line in the hero (rejected → must-fix count, flagged steps → Planner, step x/y → Implementer, etc.); per-open-question **"copy for Teams"** button (clipboard; message includes ask-role, why-it-matters, best-guess — parser also extracts `**Best guess:**`); live-update flash on sections whose content changed; `document.title` (⏳n / ✅ + key + score) and canvas-drawn favicon status dot; theme toggle auto→light→dark persisted in localStorage `tp-theme` (sets `data-theme` on `<html>`, which overrides the token blocks); keyboard shortcuts o=open r=refresh 1/2/3=demo states; relative "loaded Xs ago" ticker; 4-step pipeline explainer on the empty state; brief canvas confetti on transition to APPROVED (never on first load, skipped under reduced-motion).
